...
DEFINE_string(local_ip_for_outbound_sockets, "", "IP to bind to when making outgoing socket connections.");
DEFINE_bool(socket_inject_short_recvs, false, "Inject short recv() responses which return less data than requested");
...
Status Socket::SetNoDelay(bool enabled) {
  ...
  if (setsockopt(fd_, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag)) == -1) {
    int err = errno;
    return Status::NetworkError(std::string("failed t set TCP_NODELAY: ") + 
                                ErrnoToString(err), Slice(), err);
  }
  return Status::OK();
}
...
Status Socket::Write(const uint8_t *buf, size_t amt, size_t*nwritten) {
  if (amt <= 0) {
    return Status::NetworkError(
              StringPrintf("invalid send of %ld bytes", amt), Slice(), EINVAL);
  }
  DCHECK_GE(fd_, 0);
...
Status Socket::BlockingWrite(const uint8_t *buf, size_t buflen, size_t *nwritten,
  ...
  while (tot_written < buflen) {
    size_t inc_num_written = 0;
    size_t num_to_write = buflen - tot_written;
...
Status Socket::Read(uint8_t *buf, size_t amt, size_t *nread) {
  if (amt <= 0) {
    return Status::NetworkError(
          StringPrintf("invalid recv of %ld bytes", amt), Slice(), EINVAL);
  }
...
Status Socket::BlockingRead(uint8_t *buf, size_t amt, size_t *nread, const MonoTime& deadline) {
  DCHECK_LE(amt, std::numeric_limits<int32_t>::max()) << "Reads > INT32_MAX not supported";
  DCHECK(nread); 
  size_t tot_read = 0;
  while (tot_read < amt) {
    size_t inc_num_read = 0;
    size_t num_to_read = amt - tot_read;
    MonoDelta timeout = deadline - MonoTime::Now();
    if (PREDICT_FALSE(timeout.ToNanoseconds() <= 0)) {
      return Status::TimedOut("");
    } 
    RETURN_NOT_OK(SetRecvTimeout(timeout));
    Status s = Read(buf, num_to_read, &inc_num_read);
