#ifndef BBOY_BASE_STATUS_H_
#define BBOY_BASE_STATUS_H_
...
#define BBOY_RETURN_NOT_OK(s) do { \
    const ::bb::Status& _s = (s);             \
...
#define BBOY_RETURN_NOT_OK_PREPEND(s, msg) do { \
    const ::bb::Status& _s = (s);                              \
...
#define BBOY_RETURN_NOT_OK_RET(to_call, to_return) do { \
    const ::bb::Status& s = (to_call);                \
    if (PREDICT_FALSE(!s.ok())) return (to_return);  \
  } while (0);

/// @brief Emit a warning if @c to_call returns a bad status.
#define BBOY_WARN_NOT_OK(to_call, warning_prefix) do { \
    const ::bb::Status& _s = (to_call);              \
    if (PREDICT_FALSE(!_s.ok())) { \
      BBOY_LOG(WARNING) << (warning_prefix) << ": " << _s.ToString();  \
    } \
  } while (0);

/// @brief Log the given status and return immediately.
#define BBOY_LOG_AND_RETURN(level, status) do { \
    const ::bb::Status& _s = (status);        \
    BBOY_LOG(level) << _s.ToString(); \
    return _s; \
  } while (0);

/// @brief If the given status is not OK, log it and 'msg' at 'level' and return the status.
#define BBOY_RETURN_NOT_OK_LOG(s, level, msg) do { \
    const ::bb::Status& _s = (s);             \
    if (PREDICT_FALSE(!_s.ok())) { \
      BBOY_LOG(level) << "Status: " << _s.ToString() << " " << (msg); \
      return _s;     \
    } \
  } while (0);
...
#define BBOY_CHECK_OK_PREPEND(to_call, msg) do { \
    const ::bb::Status& _s = (to_call);                   \
    BBOY_CHECK(_s.ok()) << (msg) << ": " << _s.ToString();  \
  } while (0);

/// @brief If the status is bad, CHECK immediately, appending the status to the
///   logged message.
#define BBOY_CHECK_OK(s) BBOY_CHECK_OK_PREPEND(s, "Bad status")

/// @brief If @c to_call returns a bad status, DCHECK immediately with
///   a logged message of @c msg followed by the status.
#define BBOY_DCHECK_OK_PREPEND(to_call, msg) do { \
    const ::bb::Status& _s = (to_call);                   \
    BBOY_DCHECK(_s.ok()) << (msg) << ": " << _s.ToString();  \
  } while (0);

/// @brief If the status is bad, DCHECK immediately, appending the status to the
///   logged 'Bad status' message.
#define BBOY_DCHECK_OK(s) BBOY_DCHECK_OK_PREPEND(s, "Bad status")
...
///
/// BBOY_HEADERS_USE_SHORT_STATUS_MACROS handles this behavioral change. When
...
#define BBOY_HEADERS_USE_SHORT_STATUS_MACROS 1

#ifdef BBOY_HEADERS_USE_SHORT_STATUS_MACROS
#define RETURN_NOT_OK         BBOY_RETURN_NOT_OK
#define RETURN_NOT_OK_PREPEND BBOY_RETURN_NOT_OK_PREPEND
#define RETURN_NOT_OK_RET     BBOY_RETURN_NOT_OK_RET
#define WARN_NOT_OK           BBOY_WARN_NOT_OK
#define LOG_AND_RETURN        BBOY_LOG_AND_RETURN
#define RETURN_NOT_OK_LOG     BBOY_RETURN_NOT_OK_LOG
#define CHECK_OK_PREPEND      BBOY_CHECK_OK_PREPEND
#define CHECK_OK              BBOY_CHECK_OK
#define DCHECK_OK_PREPEND     BBOY_DCHECK_OK_PREPEND
#define DCHECK_OK             BBOY_DCHECK_OK

// These are standard glog macros.
#define BBOY_LOG              LOG
#define BBOY_CHECK            CHECK
#define BBOY_DCHECK           DCHECK
#endif
